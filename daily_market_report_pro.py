...
def get_sector_news():
    return {
        "人工智能": {"消息": "Anthropic挖角微软高管，加大AI基建投入", "影响": "利好", "建议": "持有"},
        "半导体": {"消息": "关税政策加速国产替代进程", "影响": "利好", "建议": "加仓"},
        "创新药": {"消息": "创新药出海订单频现", "影响": "利好", "建议": "持有"},
        "电网设备": {"消息": "AI时代电力需求增长", "影响": "利好", "建议": "持有"},
        "黄金": {"消息": "国际金价震荡，美联储降息预期反复", "影响": "中性", "建议": "观望"},
        "债券": {"消息": "债市震荡走软，降准预期升温", "影响": "中性偏利好", "建议": "持有"}
    }
...
def get_macro_news():
    ...
    type_summary = df.groupby("类型").agg({"持有金额": "sum", "昨日收益": "sum"}).reset_index()
    analysis = {"配置建议": [], "风险提示": [], "调仓建议": []}
